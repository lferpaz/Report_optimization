from exchangelib import Credentials, Account, DELEGATE, Configuration,FileAttachment, Credentials, Configuration, Account, DELEGATE
...
from grafiques import main_grafiques
...
# Crear el DataFrame
df = pd.DataFrame(columns=["Codi", "Aplicació", "Tecnologia", "Resultat", "Urgent", "Té incidència", "Incidencia associada", "Observacions", "Data","Peticionari"])
...
def classify_email(correo, date):
    ...
    try:
        ...
        # Buscar si hay Responsable IMI: o Remitent :
        responsable_match = re.search(r'Responsable IMI: ([^\r\n]+)|Remitent: ([^\r\n]+)|Remitent : ([^\r\n]+)', body)
        

        responsable = next((match.group(1) or match.group(2) or match.group(3) for match in [responsable_match] if match), correo.sender.name if correo.sender else "No hi ha responsable")
        ...
        if date:
            ...
            df = df.append({
                ...
                "Data": date,
                "Peticionari": responsable
            }, ignore_index=True)

            if tecnologia == "Devops" or tecnologia == ".NET" or tecnologia == "Websphere":
                if "[Instalables+Scripts+Normal]" in subject:
                    df.loc[len(df)] = ["PRO", subject, "BBDD", resultado, urgente, check, incidencia, observaciones, date, responsable]
        return df
    except:
...
def crear_copia_excel(wb,hoja,nombreArchivo):
    #hacemos una copia de la hoja en el mismo archivo, con el nombre de la hoja +_
    try:
        ws2 = wb.copy_worksheet(wb[hoja])
        ws2.title = hoja + "_"
        ws2.protection.sheet = True
        ws2.protection.enable()
        ws2.protection.password = "1234"
        #guardamos el excel
        wb.save(nombreArchivo)
        
    except:
            sg.Popup("Error en crear copia de la hoja, asseguri's que l'arxiu Plantilla.xlsx aquesta tancat, el missatge d'error es: " + str(sys.exc_info()[1]))
...
def agregar_a_plantilla(df,fechaFinal,nomArchivo,hoja):
    try:
        ...
        if check == False:
            ...
            #creamos una copia de las hojas del excel en el mismo archivo, las ponemos como no editables y les damos formato
            crear_copia_excel(wb,hoja,nomArchivo)
        

        #guardamos el excel
        wb.save(nomArchivo)
    except:
        sg.Popup("Error en agregar a la plantilla, asseguri's que l'arxiu Plantilla.xlsx aquesta tancat, el missatge d'error es: " + str(sys.exc_info()[1]))
...
def dar_formato_excel(nombreArchivo, hoja="Tecnologies"):
    try:
        ...
        print(f"{Color.FAIL}Error en dar formato al excel, asseguri's que l'arxiu Plantilla.xlsx aquesta tancat , si volia nomes generar l'informe sense agregar a la plantilla ignoreu aquest missatge, el missatge d'error es: " + str(sys.exc_info()[1]) + f"{Color.ENDC}")
...
def set_style_to_excel(df, fechaInicio, fechaFinal):
    ...
    try:
        ...
        for r in dataframe_to_rows(dfKO, index=False, header=False):
            ...
            ws3.append([r[0],r[1],r[2],r[9],r[7]])
        ...
        sg.Popup("Error en generar l'Excel, el mensaje de error es: " + str(sys.exc_info()[1]))
...
def main():
    ...
    for i in range(3):
        print(f"{Color.FAIL}ATENCIÓ : Numero d'intents restants: " + str(3-i) + f"{Color.ENDC}")
        nombre_de_usuario = input("Introduïu el vostre nom d'usuari nominal : ").strip()
        psswd = getpass("Introduïu la contrasenya: ").strip()
       
        # Verificar que ambos campos no estén vacíos
        if not nombre_de_usuario or not psswd:
            print("Usuari o contrasenya no poden estar buits, intenta de nou..")
            continue
    ...
    if cuenta is None:
        print(f"{Color.FAIL}No s'ha pogut connectar amb el servidor, si us plau intenti-ho de nou"f"{Color.ENDC}")
        exit()

    data_folder = cuenta.inbox / "data" / "2024"

    print("Seleccioneu el rang de dates")
    ...
    try:
        ...
        print(".............................................")
        get_folder_emails_recursive(data_folder, start_date, end_date)
    except ValueError:
...
if __name__ == "__main__":
    ...
    instructions = [
         "Benvingut al programa de generació d'informes de gestió de desplegaments!",
        "1. Assegura't de tenir una connexió a Internet activa.",
        "2. Verifica que els arxius 'Plantilla.xlsx' i 'grafico.xlsx' estiguin situats al mateix directori que el programa.",
        "3. Si us plau, assegura't que els arxius estiguin tancats abans d'iniciar el programa.",
        "4. Verifica que estàs utilitzant l'última versió del document, és a dir, amb la data més recent.",
        "5. En finalitzar el procés, recorda desar els arxius a la seva carpeta corresponent.",
        "Gràcies per seguir aquestes instruccions i utilitzar el nostre programa de manera eficient!"
    ]
    ...
    input("Premi qualsevol tecla per a continuar...")
